...
const SelfAssessmentForm = (props) => {
    const { edit, formData, responseErrors, preview, grades, existingAnswer } =
        props
    const {
        finalGradeResponse,
    ...
    const finalGradeModule = editMode
        ? EditCategoryModule
        : CategoryQuestionModule
        ...
        <div>
            ...
            <SelfAssessmentSection
                infoBoxTranslationId={
                    editMode ? 'SelfAssessmentQuestionsEdit' : null
                }
            ...
            {structure.openQuestions.questions.length > 0 || editMode ? (
                <SelfAssessmentSection
                    infoBoxTranslationId={
                        editMode ? 'SelfAssessmentOpenQuestionsEdit' : null
                    }
            ...
            {structure.finalGrade.includedInAssesment || editMode ? (
                <SelfAssessmentSection
                    infoBoxTranslationId={
                        editMode ? 'SelfAssessmentFinalGradeEdit' : null
                    }
                    headers={grade}
                    headerType="grade"
