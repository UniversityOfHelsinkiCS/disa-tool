...
export const SelfAssessmentFormPage = (props) => {
  ...
  const { t } = useTranslation('translation', {
    keyPrefix: 'selfAssessmentForm.selfAssessmentFormPage',
  })
    ...
    return (
      <div>
        <Container className="SelfAssessmentFormPage" data-testid="self-assessment-form-page">
          ...
          <AssessmentMessage preview={preview} open={formData.open} edit={edit} existingAnswer={existingAnswer} t={t} />
          ...
          {edit && (
            <Button color="teal" onClick={togglePreview}>
              {preview ? t('editButton') : t('previewButton')}
            </Button>
          )}
          {preview || (!formData.open && !edit) ? null : (
            <Button positive data-testid="submit-button-top" style={{ marginBottom: '25px' }} onClick={submitFunction}>
              {!edit || newAssessment ? t('saveButton') : t('updateButton')}
            </Button>
          )}
          <SelfAssessmentForm
            edit={edit}
  ...
  if (redirect || error || ((newAssessment || edit) && notTeacher)) {
    return <Redirect to="/user" />
  }

  if (assessmentResponse.existingAnswer && !formData.open) {
    return <FeedbackPage assessmentResponse={assessmentResponse} assessmentInfo={formData} />
  }
