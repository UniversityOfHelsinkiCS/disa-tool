...
const createSetOfNonRegisteredStudents = (updatedTasks) => {
  ...
  for (let i = 0; i < nonRegisteredTasks.length; i += 1) {
    ...
    const isStudentInList = nonRegisteredStudents.find((student) => student.studentnumber === resp.studentnumber)
...
const TaskResponseEditTable = ({
  ...
  const studentTasks = allStudents.map((person) => ({
    person,
    tasks: selectedTasks.map((task) => findPersonTask(person, updatedTasks, task)),
  }))
  return (
        ...
        <Table.Header>
          ...
          <Table.Row>
            {selectedTasks.map((task) => (
              <Table.HeaderCell key={task.id}>{task.name}</Table.HeaderCell>
            ))}
          </Table.Row>
        </Table.Header>
        <Table.Body>
