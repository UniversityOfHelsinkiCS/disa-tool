...
const findPersonTask = (person, updatedTasks, task) => {
    const taskUpdated = updatedTasks.find(
        (updatedTask) =>
            person.id === updatedTask.personId && updatedTask.taskId === task.id
    )
    if (taskUpdated) {
        return {
            taskId: task.id,
            id: taskUpdated.taskId,
            text: String(taskUpdated.points),
            color: 'brown',
        }
    }
    const taskMarked = person.task_responses.find(
        (response) => response.task_id === task.id
    )
    if (taskMarked) {
        return {
            ...taskMarked,
            taskId: task.id,
            text: String(taskMarked.points),
            color: 'black',
        }
    }
...
const createSetOfNonRegisteredStudents = (updatedTasks) => {
    ...
    for (let i = 0; i < nonRegisteredTasks.length; i += 1) {
        ...
        const isStudentInList = nonRegisteredStudents.find(
            (student) => student.studentnumber === resp.studentnumber
        )
...
const TaskResponseEditTable = ({
    ...
    const selectedTasks = tasks.filter((task) =>
        task.types.find((type) => type.id === selectedType.id)
    )
    const studentTasks = allStudents.map((person) => ({
        person,
        tasks: selectedTasks.map((task) =>
            findPersonTask(person, updatedTasks, task)
        ),
    }))
    return (
            ...
            <Table compact>
                <Table.Header>
                    <Table.Row>
                        ...
                        <Table.HeaderCell>
                            <Button onClick={selectType} type={selectedType}>
                                {selectedType.name}
                            </Button>
                        </Table.HeaderCell>
                    </Table.Row>
