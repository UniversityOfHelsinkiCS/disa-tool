...
export const ListTasks = (props) => (
  <div>
    <p>
      <Icon name="checkmark" color="green" /> tehty, <Icon name="delete" color="red" /> ei tehty
    </p>
    <List divided size="tiny">
      {props.tasks.map((task) => (
        <List.Item key={task.id}>
          ...
          <List.Content>
            {task.name}{' '}
            {task.types.sort(typeSorter).map((type) => (
              <Label
                basic={!props.selectedType || type.id !== props.selectedType.id}
                size="mini"
                key={type.id}
                color={props.selectedType && type.id === props.selectedType.id ? 'green' : undefined}
              >
