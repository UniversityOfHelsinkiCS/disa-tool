...
class TaskResponseTypeTable extends Component {
    constructor(props) {
        ...
        this.state = {
            selected:
                props.typeHeaders && props.typeHeaders[0]
                    ? props.typeHeaders[0].id
                    : 1,
        }
    }
    ...
    render() {
        ...
        const { typeHeaders, students, tasks, selectType, updatedTasks } =
            this.props
        const updatedHeaders = typeHeaders
            ? typeHeaders.map((header) => ({
                  ...
                  types: header.types.map((type) => ({
                      ...type,
                      updated: this.getTasksForType(tasks, type.id).find(
                          (task) =>
                                        ...
                                        header.types.map((type) => (
                                            <Table.Cell key={type.id}>
                                                {person.task_responses &&
                                                    person.task_responses.filter(
                                                        (response) =>
                                                            this.getTasksForType(
                                                                tasks,
                                                                type.id
                                                            ).find(
                                                                (t) =>
