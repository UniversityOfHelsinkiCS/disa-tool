...
class TaskResponseEdit extends Component {
    ...
    markTask = async (e, { task, person }) => {
        ...
        const taskUpdated = updatedTasks.find(
            (t) => t.taskId === task.id && t.personId === person.id
        )
        if (taskUpdated) {
            await this.setState({
                popUp: { show: true, task: taskUpdated, person },
            })
    ...
    updateTask = (e, { task }) => {
        switch (e.target.name) {
            case 'input':
                ...
                break
            case 'update': {
                const filteredTasks = this.state.updatedTasks.filter(
                    (et) =>
                        et.taskId !== task.taskId ||
                        et.personId !== task.personId
                )
    ...
    render() {
        ...
        return (
            <Grid style={{ overflowX: 'auto' }}>
                <Prompt
                    when={updatedTasks.length > 0}
                    message="Sinulla on tallentamattomia muutoksia"
                />
                <Grid.Row>
                    <Grid.Column>
                        <Accordion
...
export default connect(mapStateToProps, {
    dispatchTaskResponses: postTaskResponseActions,
})(TaskResponseEdit)
