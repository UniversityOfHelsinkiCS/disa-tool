...
export const EditOrNewForm = (props) => {
  ...
  const selectedSelfAssessments = selfAssessments.filter((s) => s.course_instance_id === parseInt(dropDownValue, 10))
      ...
      <Form>
        ...
        <Form.Field>
          {selectedSelfAssessments.length > 0 && (
            <SelfAssessmentList onClick={handleSubmit} selfAssessments={selectedSelfAssessments} />
          )}
        </Form.Field>
        <Form.Field>
          <AssessmentButtons selectedView={selectedView} onClick={handleSubmit} value={dropDownValue} />
        </Form.Field>
      </Form>
    </div>
  )
