...
const CsvTaskMapping = ({ activeCourse, activeType, csv, csvMappings, handleMapTask, toggleCsvHeader }) => (
  <List size="small">
      ...
      <List.Item key={suggestion}>
        <List.Content style={{ padding: 0 }}>
          ...
          <Dropdown
            ...
            value={csvMappings[suggestion].task.id ? csvMappings[suggestion].task.id : null}
            placeholder="Valitse vastaava tehtävä"
            options={activeCourse.tasks
              .filter((task) => activeType === 0 || task.types.find((type) => type.id === activeType))
              .map((task) => ({ key: task.id, text: task.name, value: task.id }))}
