...
const PointMapping = (props) => {
  ...
  const { t } = useTranslation('translation', {
    keyPrefix: 'userPage.uploadResponses.pointMapping',
  })
      ...
      <List>
        {Object.keys(pointsMapping).map((key) => (
          <List.Item key={key}>
            ...
            <Button basic color="red" icon="delete" size="tiny" value={key} onClick={props.removePointMapping} />
          </List.Item>
        ))}
      </List>
      <Input name="pointKey" label={t('key')} type="text" value={pointKey} onChange={handleChange} />
      <Input name="pointValue" label={t('value')} type="number" value={pointValue} onChange={handleChange} />
