...
const Nav = () => {
  ...
  return (
    <nav data-testid="navbar">
      <Menu tabular>
        <Menu.Item header as={Link} to="/" name="home" active={activeItem === 'home'} onClick={handleClick}>
          {t('home')}
        </Menu.Item>
        {user.id ? (
          <Menu.Item as={Link} to="/user" name="user" active={activeItem === 'user'} onClick={handleClick}>
            {t('user')}
          </Menu.Item>
        ) : undefined}
        <Menu.Item as={Link} to="/courses" name="courses" active={activeItem === 'courses'} onClick={handleClick}>
          {t('courses')}
        </Menu.Item>
        <Menu.Menu position="right">
          <Menu.Item>
            <Dropdown options={languageOptions} value={language} onChange={changeLanguage} />
          </Menu.Item>
          {user.role === 'ADMIN' ? (
            <Menu.Item as={Link} to="/admin" name="admin" active={activeItem === 'admin'} onClick={handleClick}>
              {t('admin')}
            </Menu.Item>
          ) : null}
          ...
          {process.env.NODE_ENV === 'development' ? (
            <Menu.Item>
              <Input defaultValue={window.localStorage.getItem('fakeShibbo')} onChange={handleFakeUser} />
            </Menu.Item>
          ) : null}
        </Menu.Menu>
      </Menu>
