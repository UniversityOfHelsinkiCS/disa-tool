...
class EditTaskObjectivesForm extends Component {
    ...
    changeModified = (id, modified) => () =>
        this.setState({
            values: {
                ...this.state.values,
                [id]: {
                    multiplier: modified
                        ? this.state.values[id].multiplier
                        : this.props.defaultMultiplier,
                    modified,
                },
            },
        })
    ...
    loadDetails = async () => {
        ...
        const details = (
            await this.props.objectivesDetails({ id: this.props.taskId })
        ).data.data
        this.setState({
            loading: false,
...
const mapStateToProps = (state, ownProps) => {
    const taskObjectives = state.task.tasks
        ...
        .objectives.reduce(
            (acc, curr) => ({
                ...acc,
                [curr.id]: { multiplier: curr.multiplier },
            }),
            {}
        )
    const objectives = state.category.categories.reduce(
        (acc, curr) =>
            acc.concat(
                curr.skill_levels.reduce(
                    (acc2, curr2) =>
                        acc2.concat(
                            curr2.objectives
                                .filter(
                                    (objective) => taskObjectives[objective.id]
                                )
                                .map((objective) => ({
                                    ...objective,
                                    multiplier:
                                        taskObjectives[objective.id].multiplier,
                                }))
                        ),
                    []
                )
    ...
    return {
        ...
        objectives,
        defaultMultiplier: state.task.tasks
            .find((task) => task.id === ownProps.taskId)
            .types.reduce(
                (acc, typeId) =>
                    acc *
                    state.type.headers.reduce((multiplier, header) => {
                        const type = header.types.find(
                            (htype) => htype.id === typeId
                        )
