...
const ResponseAccordion = ({ response }) => {
  const selfAssesmentId = useSelector((state) => state.selfAssesmentList.selfAssesmentId)
  const { t } = useTranslation('translation', { keyPrefix: 'selfAssessmentList.selfAssessmentListPage' })
      ...
      panels={[
        {
          ...
          content: (
            <Accordion.Content key={response.id}>
              <Button as={Link} to={`/selfassessment/list/${selfAssesmentId}/${response.id}`} target="_blank" basic>
                <span>{t('inspect')}</span>
              ...
              {response.response.assessmentType === 'category' ? (
                <Table collapsing compact="very">
                  <Table.Header>
                    <Table.Row>
                      <Table.HeaderCell style={{ textTransform: 'capitalize' }}>{t('category')}</Table.HeaderCell>
                      <Table.HeaderCell textAlign="center">{t('self_assessment')}</Table.HeaderCell>
                      <Table.HeaderCell textAlign="center">{t('machine_review')}</Table.HeaderCell>
                      <Table.HeaderCell textAlign="center">{t('difference')}</Table.HeaderCell>
                    </Table.Row>
                  </Table.Header>
                  <Table.Body>
                    {response.response.questionModuleResponses.map((qmResponse) => {
                      ...
                      const diff = verification.categoryVerifications.find((c) => qmResponse.id === c.categoryId)
...
export default connect()(ResponseAccordion)
