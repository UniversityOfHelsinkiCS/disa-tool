...
const formatToCsv = (responses) => {
    const formatted = responses.map((response) => {
        const findCalculatedGrade = (question) => {
            ...
            const category = verification
                ? verification.categoryVerifications.find(
                      (c) => c.categoryId === question.id
                  )
                : { earnedGrade: { name: '' } }
            return category.earnedGrade.name
        }
        const questionResponses = response.response.questionModuleResponses.map(
            (question) =>
                ...
                question.responseText
                    ? {
                          [`${question.name}_text`]: replaceQuotesAndLineBreaks(
                              question.responseText
                          ),
                          [`${question.name}_grade`]: question.grade_name,
                          [`${question.name}_calculated_grade`]:
                              findCalculatedGrade(question),
                      }
                    : {
                          [question.name]: objectiveGrades()[question.grade],
                      }
        )
        const openResponses = response.response.openQuestionResponses.map(
            (question) => ({
                [`${question.name}_text`]: replaceQuotesAndLineBreaks(
                    question.responseText
                ),
        ...
        const finalResponse = finalGradeResponse.name
            ? {
                  [`${finalGradeResponse.name}_text`]:
                      replaceQuotesAndLineBreaks(
                          finalGradeResponse.responseText
                      ),
                  [`${finalGradeResponse.name}_grade`]:
            ...
            : {}
        const flattenedQuestions = questionResponses.reduce(
            (acc, curr) => ({ ...acc, ...curr }),
            {}
        )
        const flattenedOpens = openResponses.reduce(
            (acc, curr) => ({ ...acc, ...curr }),
            {}
        )
        return {
