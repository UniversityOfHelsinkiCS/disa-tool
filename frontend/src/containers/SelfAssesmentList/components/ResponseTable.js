...
export const calculateDifference = (response) => {
    ...
    const mean = categoryVerifications.reduce(
        (acc, cur) => acc + cur.wantedGrade.difference / length,
        0
    )
    const sd = Math.sqrt(
        (1 / (length - 1)) *
            categoryVerifications.reduce(
                (acc, cur) => acc + (cur.wantedGrade.difference - mean) ** 2,
                0
            )
    )
    return { mean, sd }
...
const responseDifferences = (response) => {
    ...
    return (
        <Table.Cell
            collapsing
            positive={diff.mean < 0}
            negative={diff.mean > 0}
        >
