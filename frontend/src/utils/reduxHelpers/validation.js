...
export const maxLength = (toCheck, toCheckAttribute, max, acc) =>
  toCheck[toCheckAttribute].length > max
    ? [...acc, { id: toCheck.id, error: `${errorMessages.max[lang]} ${max}` }]
    : acc
export const minLength = (toCheck, toCheckAttribute, min, acc) =>
  toCheck[toCheckAttribute].length < min
    ? [...acc, { id: toCheck.id, error: `${errorMessages.min[lang]} ${min}` }]
    : acc
...
export const checkResponseErrors = (assessmentResponse) => {
  const { questionModuleResponses, openQuestionResponses, finalGradeResponse, assessmentType } = assessmentResponse
      ...
      if (!qmRes.grade) {
        if (grade.find((qm) => qm.id === qmRes.category)) {
          ...
          grade = [
            ...
            {
              id: qmRes.category,
              errors: { [qmRes.id]: { error: errorMessages.exists[lang] } },
            },
          ]
        }
      }
