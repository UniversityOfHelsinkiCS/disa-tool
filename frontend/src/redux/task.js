    ...
    return {
        ...
        tasks: state.tasks.map((task) => {
            if (deleteFrom[task.id]) {
                return {
                    ...task,
                    types: task.types.filter(
                        (type) => type !== action.response.deleted.id
                    ),
                }
            }
            return task
...
const detachManyObjectives = (state, action) => ({
    ...state,
    tasks: state.tasks.map((task) => {
        const deletionTask = action.response.deleted.tasks.find(
        ...
        return {
            ...task,
            objectives: task.objectives.filter(
                (objective) => !toDelete[objective.id]
            ),
        }
    }),
})
    ...
    tasks: state.tasks.map((task) => {
        if (task.id === action.response.deleted.task_id) {
            return {
                ...
                types: task.types.filter(
                    (type) => type !== action.response.deleted.type_id
                ),
                objectives: [
                    ...task.objectives.filter(
                        (objective) =>
                            !action.response.taskObjectives.find(
                                (taskObjective) =>
                                    taskObjective.id === objective.id
                            )
                    ),
    ...
    switch (action.type) {
        ...
        case types.TASK_DELETE:
            return {
                ...
                tasks: state.tasks.filter(
                    (task) => task.id !== action.response.deleted.id
                ),
            }
        ...
        case types.TASK_EDIT:
            return {
                ...state,
                tasks: state.tasks.map((task) =>
                    task.id === action.response.edited.id
                        ? {
                              ...task,
        ...
        case types.TASK_EDIT_OBJECTIVE_MULTIPLIERS:
            return {
                ...state,
                tasks: state.tasks.map((task) =>
                    task.id === action.response.edited.task_id
                        ? {
                              ...task,
                              objectives: task.objectives.map((objective) => {
                                  const editedObjective =
                                      action.response.edited.task_objectives.find(
                                          (to) =>
                                              to.objective_id === objective.id
                                      )
                                  return editedObjective
                                      ? {
                                            ...objective,
                                            multiplier:
                                                editedObjective.multiplier,
                                        }
                                      : objective
                              }),
                          }
