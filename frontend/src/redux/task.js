...
const detachOneObjective = (state, action) => ({
  ...
  tasks: state.tasks.map((task) => {
    if (task.id === action.response.deleted.task_id) {
      return {
        ...task,
        objectives: task.objectives.filter((objective) => objective.id !== action.response.deleted.objective_id),
      }
    }
    return task
  }),
...
const taskReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case types.TASK_ATTACH_TYPE:
      return attachType(action.response.deleted ? detachOneType(state, action) : state, action)
