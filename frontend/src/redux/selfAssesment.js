...
import {
    ...
    initResponseForm,
    respond,
} from '../utils/reduxHelpers/selfAssesment'
    ...
    switch (action.type) {
      ...
      */
            ...
            return {
                ...
                assesmentResponse: {
                    ...
                    openQuestionResponses:
                        state.assesmentResponse.openQuestionResponses.map(
                            (oQ) =>
                                oQ.id === id
                                    ? { ...oQ, responseText: value }
                                    : oQ
                        ),
                },
            }
            ...
            return {
                ...
                createForm: {
                    ...
                    structure: {
                        ...
                        questionModules: [
                            ...state.createForm.structure.questionModules.map(
                                (o) =>
                                    o.id !== id
                                        ? o
                                        : { ...o, textFieldOn: !o.textFieldOn }
                            ),
                        ],
                    },
                },
        ...
        case types.TOGGLE_DOWN: {
            const { a, b, questionModules } = getToggleProps(
                action.payload,
                state
            )
                        ...
                        openQuestions: {
                            ...
                            incrementId:
                                state.createForm.structure.openQuestions
                                    .incrementId + 1,
                            questions: [
                                ...state.createForm.structure.openQuestions.questions.concat(
                                    data
                                ),
                            ],
                        },
                ...
                createForm: {
                    ...
                    structure: {
                        ...
                        openQuestions: {
                            ...state.createForm.structure.openQuestions,
                            questions:
                                state.createForm.structure.openQuestions.questions.filter(
                                    (oQ) => oQ.id !== id
                                ), //eslint-disable-line
                        },
                    },
        ...
        case types.CHANGE_TEXT_FIELD: {
            ...
            const toChange = state.createForm.structure.formInfo.find(
                (h) =>
                    h.type ===
                    (type === 'instructions' ? langInstructions : langName)
            )
            let value = null
            ...
            return {
                ...
                createForm: {
                    ...
                    structure: {
                        ...state.createForm.structure,
                        formInfo: state.createForm.structure.formInfo.map(
                            (inst) =>
                                ids.includes(inst.id.toString())
                                    ? { ...inst, value: values[inst.id] }
                                    : inst
                        ),
                    },
                    [type]: value,
        ...
        case types.CREATE_SELF_ASSESMENT_SUCCESS: {
            const selfAssesments = state.userSelfAssesments.concat(
                action.payload.data
            )
            return {
                ...state,
                createForm: [],
                userSelfAssesments: selfAssesments,
            }
        }
        ...
        case types.CHANGE_HEADER: {
            ...
            if (headerType) {
                let name = state.createForm.structure.headers[headerType].find(
                    (h) => h.type === langName
                )
                name = ids.includes(name.id.toString())
                    ? changedHeaders[name.id]
                    : name.value
            ...
            let name = state.createForm.structure.finalGrade.headers.find(
                (h) => h.type === langName
            )
            name = ids.includes(name.id.toString())
                ? changedHeaders[name.id]
                ...
                createForm: {
                    ...
                    structure: {
                        ...
                        finalGrade: {
                            ...state.createForm.structure.finalGrade,
                            headers:
                                state.createForm.structure.finalGrade.headers.map(
                                    (fg) =>
                                        ids.includes(fg.id.toString())
                                            ? {
                                                  ...fg,
                                                  value: changedHeaders[fg.id],
                                              }
                                            : fg
                                ),
                            name,
        ...
        case types.SELF_ASSESMENT_UPDATE_SUCCESS: {
            ...
            return {
                ...state,
                userSelfAssesments: state.userSelfAssesments.map((s) =>
                    s.id === data.id ? data : s
                ),
            }
        }
