...
const objectiveEdit = (state, edited) => {
    ...
    if (
        oldLevel.id === edited.skill_level_id &&
        oldCategory.id === edited.category_id
    ) {
        return {
            ...state,
                    ...
                    ? {
                          ...
                          skill_levels: category.skill_levels.map((level) =>
                              level.id === edited.skill_level_id
                                  ? {
                                        ...level,
                                        objectives: level.objectives.map(
                                            (objective) =>
                                                objective.id === edited.id
                                                    ? newObjective
                                                    : objective
                                        ),
                                    }
    ...
    switch (action.type) {
            ...
            return {
                ...
                categories: state.categories.map((category) => {
                    if (category.id === action.response.created.category_id) {
                        return {
                            ...category,
                            skill_levels: category.skill_levels.map(
                                (skillLevel) => {
                                    if (
                                        skillLevel.id ===
                                        action.response.created.skill_level_id
                                    ) {
                                        return {
                                            ...skillLevel,
                                            objectives: [
                                                ...skillLevel.objectives,
                                                {
                    ...
                    category.id === action.response.deleted.category_id
                        ? {
                              ...
                              skill_levels: category.skill_levels.map(
                                  (skillLevel) =>
                                      skillLevel.id ===
                                      action.response.deleted.skill_level_id
                                          ? {
                                                ...skillLevel,
                                                objectives:
                                                    skillLevel.objectives.filter(
                                                        (objective) =>
                                                            objective.id !==
                                                            action.response
                                                                .deleted.id
        ...
        case types.CATEGORY_DELETE:
            return {
                ...
                categories: state.categories.filter(
                    (category) => category.id !== action.response.deleted.id
                ),
            }
        ...
        case types.LEVEL_DELETE:
            return {
                ...
                categories: state.categories.map((category) => ({
                    ...category,
                    skill_levels: category.skill_levels.filter(
                        (level) => level.id !== action.response.deleted.id
                    ),
                })),
            }
                ...
                categories: state.categories.map((category) => ({
                    ...
                    skill_levels: category.skill_levels.map((level) => ({
                        ...
                        objectives: level.objectives.map((objective) =>
                            objective.id ===
                            action.response.created.objective_id
                                ? {
                                      ...objective,
            ...
            return {
                ...
                categories: state.categories.map((category) => ({
                    ...
                    skill_levels: category.skill_levels.map((level) => ({
                        ...
                        objectives: level.objectives.map((objective) =>
                            objective.id ===
                            action.response.deleted.objective_id
                                ? {
                                      ...objective,
                                      task_count: objective.task_count - 1,
                                  }
                ...
                categories: state.categories.map((category) =>
                    category.id === action.response.edited.id
                        ? {
                              ...category,
                              name:
                                  action.response.edited.name || category.name,
                              order:
                                  action.response.edited.order ||
                                  category.order,
                          }
                        : category
                ),
