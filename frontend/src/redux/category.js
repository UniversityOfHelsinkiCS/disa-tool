...
const objectiveEdit = (state, edited) => {
  ...
  const oldCategory = state.categories.find((category) => {
    oldLevel = category.skill_levels.find((level) => {
      oldObjective = level.objectives.find((objective) => objective.id === edited.id)
  ...
  switch (action.type) {
    ...
    case types.LEVEL_DELETE:
      return {
        ...
        categories: state.categories.map((category) => ({
          ...category,
          skill_levels: category.skill_levels.filter((level) => level.id !== action.response.deleted.id),
        })),
      }
