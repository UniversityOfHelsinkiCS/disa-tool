...
const edit = (state, edited) => {
    const oldHeader = state.headers.find((header) =>
        header.types.find((type) => type.id === edited.id)
    )
        ...
        headers: state.headers.map((header) => {
            switch (header.id) {
                ...
                case oldHeader.id:
                    return {
                        ...header,
                        types: header.types.filter(
                            (type) => type.id !== edited.id
                        ),
                    }
                default:
                    return header
    ...
    switch (action.type) {
        ...
        case types.TYPE_DELETE:
            return {
                ...state,
                headers: state.headers.map((header) =>
                    header.id === action.response.deleted.type_header_id
                        ? {
                              ...header,
                              types: header.types.filter(
                                  (type) =>
                                      type.id !== action.response.deleted.id
                              ),
                          }
                        : header
                ),
        ...
        case types.TYPE_CREATE:
            return {
                ...state,
                headers: state.headers.map((header) =>
                    header.id === action.response.created.type_header_id
                        ? {
                              ...header,
                              types: [
                                  ...header.types,
        ...
        case types.TYPE_HEADER_DELETE:
            return {
                ...
                headers: state.headers.filter(
                    (header) => header.id !== action.response.deleted.id
                ),
            }
