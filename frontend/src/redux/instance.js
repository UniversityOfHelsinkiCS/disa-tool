...
export const instanceReducer = (state = INITIAL_STATE, action) => {
    switch (action.type) {
        ...
        case types.ASSESMENT_RESPONSE_CREATE_SUCCESS: {
            ...
            const toReplace = oldAssesments.find(
                (oA) => oA.id === data.self_assessment_id
            )
            toReplace.assessment_responses = [
                ...toReplace.assessment_responses,
                data,
            ]
            const newAssessments = oldAssesments.map((oA) =>
                oA.id === data.self_assessment_id ? toReplace : oA
            )
        ...
        case types.COURSE_INSTANCE_UPDATE_PERSON_SUCCESS: {
            ...
            action.payload.updatedPeople.forEach((coursePerson) => {
                const updated = currentPeople.find(
                    (person) => coursePerson.person_id === person.id
                )
                if (updated) {
                    updated.course_instances[0].course_person.role =
                        coursePerson.role
                }
            })
            action.payload.newPeople.forEach((person) => {
                const updated = currentPeople.find(
                    (oldPerson) => person.id === oldPerson.id
                )
                if (!updated) {
        ...
        case types.COURSE_INSTANCE_POST_TASK_RESPONSES_SUCCESS: {
            ...
            updatedTasks.forEach((task) => {
                ...
                if (student) {
                    const oldTask = student.task_responses.find(
                        (t) => t.id === task.id
                    )
                    if (oldTask) {
        ...
        case types.COURSE_INSTANCE_DELETE_PERSON_SUCCESS: {
            ...
            const updatedPeople = state.people.filter(
                (person) => person.id !== deleted.person_id
            )
        ...
        case types.SELF_ASSESMENT_UPDATE_SUCCESS:
            return {
                ...
                self_assessments: state.self_assessments.map((assesment) =>
                    assesment.id === action.payload.data.id
                        ? action.payload.data
                        : assesment
                ),
            }
