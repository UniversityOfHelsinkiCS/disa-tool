...
test.describe("Task objective tests", () => {
    ...
    test('Edit objective', async ({ page }) => {
        ...
        await page.getByRole('cell', { name: 'Tunnen vektorin määritelmän lukujonona ja osaan havainnollistaa tason vektoreita pisteinä tai suuntajanoina' }).getByRole('button').nth(0).click()
        await page.locator('input[name="name-input"]').click()
    ...
    test('Remove objective', async ({ page }) => {
        ...
        await page.getByRole('textbox').press('Enter');
        await page.goto('http://localhost:8080/course/1/matrix');
        await expect(page.locator("[data-testid='matrix-level-3-1']")).toHaveScreenshot( 'remove-objective-module-before.png' )
        await page.getByRole('cell', { name: 'Osaan selvittää, onko vektori toisten vektorien lineaarikombinaatio' }).getByRole('button').nth(1).click()
        ...
        await expect(page.getByText('Osaan selvittää, onko vektori toisten vektorien lineaarikombinaatio')).not.toBeVisible()
    })
})