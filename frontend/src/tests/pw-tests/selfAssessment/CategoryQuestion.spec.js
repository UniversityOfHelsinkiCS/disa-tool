...
test.only('test', async ({ page }) => {
  ...
  await expect(page.locator("[data-testid='self-assessment-form-page']")).toHaveScreenshot({ path: 'self-assessment-category-page.png' });
  ...
  await expect(page.locator("div[data-testid='edit-category-module-10']")).toHaveScreenshot({ path: 'edit-category-module-before-clicks.png' });
  ...
  await expect(page.locator("[data-testid='edit-category-module-10']")).toHaveScreenshot({ path: 'edit-category-module-after-clicks.png' });
...
// Can be toggled back
  ...
  await expect(page.getByTestId('self-assessment-section-list-Kurssin osa-alueet').getByRole("listitem").nth(0)).toContainText('Yhtälöryhmät');
  await page.getByTestId('toggle-down-button-10').click();
  await expect(page.getByTestId('self-assessment-section-list-Kurssin osa-alueet').getByRole("listitem").nth(0)).toContainText('Vektoriavaruudet');
  await page.getByTestId('toggle-up-button-12').click();
  await expect(page.getByTestId('self-assessment-section-list-Kurssin osa-alueet').getByRole("listitem").nth(1)).toContainText('Virittäminen ja vapaus');
  ...
  await expect(page.getByTestId("teacher-assessment-list")).toHaveScreenshot({ path: 'course-page-course-info-after.png' })
});