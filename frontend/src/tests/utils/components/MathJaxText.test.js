...
const content =
    'text text $ e = mc^2 $ more text $ a = %% a %% $ final %% text %%.'
const delimiters = ['$']
...
describe('MathJaxText component', () => {
    ...
    beforeEach(() => {
        wrapper = shallow(
            <MathJaxText content={content} delimiters={delimiters} />
        )
    })
    it('renders spans.', () => {
    ...
    it('renders plain text plainly.', () => {
        content
            .split(/\$.+\$/)
            .forEach((text) =>
                expect(findText(text, wrapper)).toBeGreaterThan(0)
            )
    })
    ...
    it('renders delimited text as MathJax.Node formulae.', () => {
        const formulae = wrapper
            .find(MathJax.Node)
            .map((element) => element.prop('formula'))
        content.split('$').forEach((text, i) => {
            if (i % 2 === 0) return
    ...
    it('does not render delimiters.', (done) => {
        ...
        formulae.forEach((formula) => {
            if (formula.includes(delimiters[0]))
                done('MathJax.Node element rendered delimiter.')
        })
        const foundDelimiters = findText(delimiters[0], wrapper)
        if (foundDelimiters > 0)
            done(
                `span elements rendered ${foundDelimiters} delimiters. Expected 0.`
            )
        done()
    })
    ...
    describe('with multiple delimiters', () => {
        ...
        it('Only splits outside earlier mathjax blocks.', () => {
            const formulae = wrapper
                .find(MathJax.Node)
                .map((element) => element.prop('formula'))
            const found = formulae.reduce(
                (acc, curr) => acc + (curr.split('%%').length - 1),
                0
            )
